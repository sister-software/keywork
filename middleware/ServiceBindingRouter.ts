...
import { RequestRouter, RequestRouterOptions, RouteRequestHandler, WorkerEnvFetchBinding } from 'keywork/router'
...
> {
  ...
  private onRequest: RouteRequestHandler<Record<BindingAlias, WorkerEnvFetchBinding>> = ({ env, request }) => {
    ...
    if (!proxiedBinding) {
      const publicError = `Binding \`${this.bindingAlias}\` of \`${proxiedBinding}\` is not present in \`env\``
