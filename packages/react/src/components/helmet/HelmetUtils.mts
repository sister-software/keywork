...
import {
  ...
  TAG_NAMES,
  TAG_PROPERTIES,
} from './HelmetConstants.mjs'
import { HelmetData, HelmetDatum, HelmetPropsToState, HelmetTags, OtherElementAttributes } from './types.mjs'
...
declare global {
  ...
  interface Document {
    ...
    createTextNode(textContent: string): any
...
const getBaseTagFromPropsList = (primaryAttributes: string[], propsList: any[]) => {
  return propsList
...
const getTagsFromPropsList = (tagName: string, primaryAttributes: string[], propsList: any[]) => {
  // Calculate list of tags, giving priority innermost component (end of the propslist)
  const approvedSeenTags: Record<string, any> = {}
  ...
  return propsList
    ...
    .reduce((approvedTags, instanceTags) => {
      const instanceSeenTags: Record<string, any> = {}
      ...
      instanceTags
        ...
        .reverse()
        .forEach((tag: any) => approvedTags.push(tag))
...
export const reducePropsToState = (propsList: any[]) => ({
...
let _helmetCallback = -1

export const handleClientStateChange = (newState: HelmetPropsToState) => {
  if (_helmetCallback) {
...
const commitTagChanges = (newState: NonNullable<HelmetPropsToState>, cb?: () => void) => {
  const {
    baseTag,
    bodyAttributes,
    htmlAttributes,
  ...
  updateTitle(title, titleAttributes as any)
  ...
  Object.keys(tagUpdates).forEach((tagType) => {
    ...
    if (newTags.length) {
      ;(addedTags as any)[tagType] = newTags
    }
    if (oldTags.length) {
      ;(removedTags as any)[tagType] = tagUpdates[tagType].oldTags
    }
  })
...
const updateTitle = (title: string | undefined, attributes: OtherElementAttributes = {}) => {
...
const updateAttributes = (tagName: string, attributes: OtherElementAttributes = {}) => {
  ...
  for (let i = 0; i < attributeKeys.length; i++) {
    ...
    if (elementTag.getAttribute(attribute) !== value) {
      elementTag.setAttribute(attribute, value as any)
    }
...
const updateTags = (type: string, tags: any[]) => {
  ...
  if (tags && tags.length) {
    tags.forEach((tag) => {
      const newElement = document.createElement(type) as any
...
const generateElementAttributesAsString = (attributes: OtherElementAttributes) =>
...
export const convertReactPropstoHtmlAttributes = (props: any, initAttributes = {}) => {
  return Object.keys(props).reduce((obj, key) => {
    ;(obj as any)[(HTML_TAG_MAP as any)[key] || key] = props[key]
    return obj
  }, initAttributes)
}
...
const generateTagsAsReactComponent = (type: string, tags: any[]) =>
  tags.map((tag, i) => {
    ...
    Object.keys(tag).forEach((attribute) => {
      const mappedAttribute = (REACT_TAG_MAP as any)[attribute] || attribute
