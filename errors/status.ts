...
export function isInformationalStatus(status: Status): status is InformationalStatus {
...
export function isClientErrorStatus(status: Status): status is ClientErrorStatus {
  return status >= 400 && status < 500
}

/** A type guard that determines if the status code is a server error. */
export function isServerErrorStatus(status: Status): status is ServerErrorStatus {
