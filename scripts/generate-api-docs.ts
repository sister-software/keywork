...
const getDest = (filePath: string) => {
  return path.join(ProjectFiles.DocsModulesDirectory, filePath.substring(ProjectFiles.ModulesDirectory.length))
}
...
async function typeDocPlugin(namedExport: string, mapping: NPMExportEntry) {
  ...
  const outPath = path.join(ProjectFiles.DocsModulesDirectory, path.dirname(filePath))
...
const APIOutputDirExists = await checkFileExists(ProjectFiles.DocsModulesDirectory)

if (APIOutputDirExists) {
  await fs.rm(ProjectFiles.DocsModulesDirectory, { recursive: true, force: true })
} else {
  await fs.mkdir(ProjectFiles.DocsModulesDirectory, { recursive: true })
}
...
await fs.writeFile(
  path.join(ProjectFiles.DocsModulesDirectory, 'module-manifest.json'),
