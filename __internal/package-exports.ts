...
import { changeExtension } from 'keywork/__internal/file-utils'
import { repoRootPathBuilder } from 'keywork/__internal/paths'
import { OutBuildManifest, OutDirectory, PackageJSON } from 'keywork/__internal/project'
...
export function extractEntrypoints(packageName: string, importMap: ImportMap) {
  ...
  for (const [specifier, remappedSpecifier] of Object.entries(importMap.imports)) {
    ...
    const _import = changeExtension(relativeSpecifier, '')
